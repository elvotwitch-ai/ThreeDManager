...
public class GCodePrintFileParser : IPrintFileParser
{
    ...
    public ParsedPrintMetadata Parse(string fileName, string rawContent)
    {
        ...
        {
            SlicerName = NormalizeSlicerName(ExtractString(rawContent,
    ...
    // PrusaSlicer, SuperSlicer, and OrcaSlicer append the per-file build timestamp to the
    // "generated by" line ("... on 2026-07-15 at 10:00:00 UTC"). Left in, the same slicer produces
    // a different SlicerName for every file it exports, so imports cannot be grouped by slicer.
    // The trailing " on <ISO date> ..." is unambiguous — no real slicer name contains an ISO date —
    // so stripping it keeps the name and version while removing only the noise. Cura ("generated
    // with Cura_SteamEngine 5.4.0") and the Creality line carry no such suffix and are untouched.
    private static string? NormalizeSlicerName(string? value)
    {
